...
datetime timeBUOVB_BEOVB = 0; // For tracking the last detected pattern

// Function to check for Bullish or Bearish Engulfing Pattern
bool CheckEngulfingPattern()
{
    double _bid = NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits); // Define Low price 
    double _ask = NormalizeDouble(SymbolInfoDouble(_Symbol, SYMBOL_ASK), _Digits); // Define High price
    ...
    double bar1Size = NormalizeDouble((high1 - low1) / _point, 0); // Size of the first bar

    // Check for Bearish Engulfing Pattern
    if (timeBUOVB_BEOVB != iTime(_Symbol, _Period, 1) && // Orders are not yet opened for this pattern 
        bar1Size > 0 && // First bar is big enough
    ...
    {
        timeBUOVB_BEOVB = iTime(_Symbol, _Period, 1); // Indicate a bearish pattern detected
        return true; // Return true for a bearish engulfing
    }
    ...
    {
        timeBUOVB_BEOVB = iTime(_Symbol, _Period, 1); // Indicate a bullish pattern detected
        return true; // Return true for a bullish engulfing
    }

    return false; // No engulfing pattern detected
}
...
void OnTick()
{
    ...
    if (tradePlaced == false && CheckEngulfingPattern())
    {
